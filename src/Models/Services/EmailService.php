...
use WalkerChiu\Newsletter\Models\Services\EmailSenderJob;
...
class EmailService
{
    ...
    {
        $this->repository = App::make(config('wk-core.class.newsletter.emailRepository'));
        $this->channels = [];
    }
    ...
    public function categorizeReceivers($site, $receiver): array
    {
        ...
        if (config('wk-newsletter.onoff.account')) {
    ...
    {
        $collection = [];
        if (!is_iterable($users)) $users = [$users];
    ...
    public function buildCard($entity, $addressTypes, string $language): array
    {
        ...
        if (
            $addressTypes != 'user'
            && config('wk-newsletter.onoff.morph-address')
        ) {
            $address = null;
            ...
            foreach ($addressTypes as $type) {
                if ($type == 'site') {
                    ...
                    if (
                        config('wk-newsletter.onoff.account')
                        && $entity->profile
                    ) {

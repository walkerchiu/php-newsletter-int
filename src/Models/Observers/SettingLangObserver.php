...
namespace WalkerChiu\Newsletter\Models\Observers;
...
class SettingLangObserver
{
    ...
    {
        $query =
            config('wk-core.class.newsletter.settingLang')
                ::withTrashed()
        ...
        if (
            config('wk-newsletter.soft_delete')
            && (
                config('wk-core.lang_log')
                || config('wk-newsletter.lang_log')
            )
        ) {
